...
import { newInvoice } from "../actionsMap"

import Spinner from "./Spinner"

const FormAddInvoices = () => {
    ...
    const [title, setTitle] = useState(`Invoice of  ${current.getDate()}/${current.getMonth() + 1}/${current.getFullYear()}`);
    ...
    const createInvoice = async () => {
        ...
        if (allFilled) {
            setError(false)
            await dispatch(newInvoice({ userId: id, title, articulos: inputs, pagado: pagado }))
            router.push('/')
...
export default FormAddInvoices;